...
sub cmd_crapbusterjpq($$$) {
  my ($args, $server, $witem) = @_;
  my $old = Irssi::settings_get_str('crapbuster_levels');
  my $jpq = 'JOINS PARTS QUITS';
  Irssi::settings_set_str('crapbuster_levels',$jpq);
  cmd_crapbuster($args,$server,$witem);
  Irssi::settings_set_str('crapbuster_levels',$old);
}
...
Irssi::command_bind('crapbusterjpq', \&cmd_crapbusterjpq);
